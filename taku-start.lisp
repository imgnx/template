...
(defun echo (&rest parts) 
  (format t "~&[taku] ~{~a~}~%" parts)) 

(defun spawn 
  (argv &key (dir (repo-root))) 
  (format t "~&[taku] spawn: ~{~a~^ ~}~%" argv) 
...
(defun start-frontend 
  ...
  (ensure-bins "npm")
  (let* ((proc (spawn (cmd-frontend target) :dir root)))
    ;; Wait for process to finish
    (wait-any proc)
    ;; Check exit status
    (unless (zerop (process-exit-code proc))
      (echo "Frontend failed. Dumping frontend.log:")
      (let ((logfile (merge-pathnames "frontend.log" root)))
        (echo (uiop:read-file-string logfile))))
    proc))
